...
const Dashboard = ({ selectedDate, onDateChange, onOpenWeightLogger }) => {
  ...
  const styles = {
    ...
    statValue: { fontSize: '32px', fontWeight: '700', background: 'var(--accent-gradient)', WebkitBackgroundClip: 'text', WebkitTextFillColor: 'transparent', marginBottom: '4px' },
    ...
    <div style={styles.container}>
      ...
      <div style={styles.statsGrid}>
        <div style={styles.statCard}>
          <div style={styles.statHeader}><Flame size={20} style={styles.statIcon} /><span style={styles.statLabel}>摄入热量</span></div>
          <div style={styles.statValue}>{caloriesConsumed}</div>
          <div style={styles.statSubtext}>目标: {calorieTarget.toFixed(0)} kcal</div>
        </div>
        <div style={styles.statCard}>
          <div style={styles.statHeader}><Drumstick size={20} style={styles.statIcon} /><span style={styles.statLabel}>蛋白质</span></div>
          <div style={styles.statValue}>{proteinConsumed}g</div>
          <div style={styles.statSubtext}>目标: {proteinTarget.toFixed(0)}g</div>
        </div>
        <div style={styles.statCard}>
          <div style={styles.statHeader}><Dumbbell size={20} style={styles.statIcon} /><span style={styles.statLabel}>运动消耗</span></div>
          <div style={styles.statValue}>{exerciseCalories}</div>
          <div style={styles.statSubtext}>kcal</div>
        </div>
        <button type="button" style={styles.statCard} onClick={onOpenWeightLogger} aria-label="打开体重记录">
          <div style={styles.statHeader}><Scale size={20} style={styles.statIcon} /><span style={styles.statLabel}>当前体重</span></div>
          <div style={styles.statValue}>{profile.currentWeight || 0}</div>
          <div style={styles.statSubtext}>点击记录新体重</div>
        </button>
      </div>
      <div style={{ ...styles.actionCard, ...(actionPlan.tone === 'warning' ? styles.actionCardWarning : actionPlan.tone === 'calm' ? styles.actionCardCalm : {}) }}>
